...
* Now you need to call the game's update method in a loop like this:
...
The Game object is the foundation for Crown.JS and mainly exists to render and create actors.
...
Similar to adding an actor, this draws the actor to the offScreenCanvas. It can be used for things like background images or other static things.
...
This function in f will be called immediately and is more for looks than anything.
...
* rot: the rotation of the sprite
...
Creates an event and calls the function f every frame where the key is down.
...
This object is pretty separate from the rest of Crown.JS just made for playing sounds.
...
Formats a number so that it is an n-character string.
...
TWEEN algorithms are a set of functions that all take in the value of t and returns a new value. They can be used to modify the behavior of a TWEEN. It is recommended you use the [grapher.js](https://github.com/KingstumusPrime/--Crown.JS/tree/main?tab=readme-ov-file#grapherjs) example to see how they behave.
...
* bounceOut: adds a  bounce at the end

* smoothstep: the smoothstep function

* easeOutQuint: ease out

* easeInBounce: bounce is at the start instead of the end

* elasticIn: equivalent of easeInBounce but with elasticOut
...
In order to add multiplayer, you need to include the client.js file in your project. After that, you need to create a server. See the sample server also included in the /multiplayer folder. Once your server is up, update the value of PEER_JS_SERVER_URL and PEER_JS_SERVER_NAME.
...
Now that you have that, the client needs to fill out a couple of functions:
...
Then you're done! To send data to the server, simply call:
